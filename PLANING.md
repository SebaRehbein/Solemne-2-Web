...
* [ ] Pruebas de compatibilidad cross-browser: ejecutar el juego en Firefox y Safari, registrar y corregir diferencias de comportamiento respecto a Chrome (eventos de teclado, canvas rendering, estilos CSS).
...
* [ ] Pruebas unitarias del frontend (Solemne 2): implementar tests sobre el sistema de puntaje ya funcional y sobre el guardado en localStorage (guardar, recuperar y limpiar partida). Usar Jest o Vitest según el setup existente.
...
* [ ] POST /api/admin/login → valida credenciales de admin, retorna { token: <JWT> } en el body (sin cookie)
...
* [ ] Pantalla de Registro y Login para jugadores (el navegador maneja la cookie automáticamente)
